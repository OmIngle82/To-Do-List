...
const updateUIforLoginState = (user) => {
    ...
    if (user) {
        ...
        listenForProfile();
        listenForTasks();
        setInterval(checkTaskReminders, 60000); // Check for reminders every minute
    } else {
...
const updateTaskCounters = () => {
    ...
    if (statPending) statPending.textContent = total - completed;
};
...
const handleVoiceInput = () => {
    if (recognition) {
        ...
        recognition.onresult = (event) => {
            ...
            taskInput.dispatchEvent(new Event('keyup')); // Trigger AI suggestion
        };
    } else {
...
const checkTaskReminders = () => {
    ...
    allTasks.forEach(task => {
        if (task.deadline && (task.status !== 'completed' && !task.completed)) {
            const deadline = new Date(task.deadline.seconds * 1000);
            if (deadline > now && deadline <= reminderWindow) {
...
const getAISuggestion = (text) => {
    ...
    if (/\b(buy|get|milk|bread|market)\b/.test(text)) return 'Shopping';
    if (/\b(meet|call|report|email|presentation|deadline)\b/.test(text)) return 'Work';
    if (/\b(read|assignment|exam|learn|chapter)\b/.test(text)) return 'Study';
    return null;
};
...
taskForm?.addEventListener('submit', (e) => {
    ...
    const deadline = deadlineDateInput.value ? firebase.firestore.Timestamp.fromDate(new Date(`${deadlineDateInput.value} ${deadlineTimeInput.value || "00:00"}`)) : null;
    ...
    const taskData = { text: taskInput.value.trim(), priority: prioritySelect.value, status: status, deadline, category, subtasks };
...
taskInput?.addEventListener('keyup', (e) => {
    ...
    const suggestion = getAISuggestion(e.target.value);
    aiSuggestionBox.innerHTML = '';
    if (suggestion) {
        const chip = document.createElement('div');
        chip.className = 'ai-suggestion-chip';
        chip.textContent = `Suggested Category: ${suggestion}`;
        chip.onclick = () => { categorySelect.value = suggestion; customCategoryInput.classList.add('hide'); };
...
if(enableNotificationsBtn) enableNotificationsBtn.addEventListener('click', () => {
    ...
    Notification.requestPermission().then(permission => {
        if (permission === 'granted') showFeedback(profileFeedback, 'Notifications enabled!', 'success');
        else showFeedback(profileFeedback, 'Notifications not granted.', 'error');
    });
});
